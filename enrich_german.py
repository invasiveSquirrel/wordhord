...
class GermanEnrichedWord(BaseModel):
    model_config = ConfigDict(extra='ignore')
    lemma: str = Field(description="The base word.")
    pos: str = Field(description="Part of speech.")
    noun_cases: Optional[str] = Field(default="", description="Nominative and genitive forms, and irregular changes in declensions.")
    verb_forms: Optional[str] = Field(default="", description="1st person singular, irregular consonant or vowel changes.")
    ...
    irregular_forms: Optional[str] = Field(default="", description="Masculine accusative forms, plural forms.")
    verb_tense_examples: Optional[Dict[str, str]] = Field(default={}, description="Present, Past, Perfect, Subjunctive I/II examples.")
    idiomatic_notes: Optional[str] = Field(default="", description="Idiomatic usage notes, phrasal verb forms, nominative expressions.")
...
async def enrich_batch(words: List[str]) -> List[dict]:
    prompt = f"""
    You are an expert German linguistic corpus analyzer. Enrich the following German words with deep morphological and syntactical data.
    Words to enrich: {", ".join(words)}

    For each word, provide a JSON object adhering to these rules:
    - lemma: The base word.
    - pos: Part of speech.
    - noun_cases: If a noun, provide Nominative and Genitive forms, and note irregular consonant/vowel changes.
    - verb_forms: If a verb, provide 1st person singular and note irregular changes.
    ...
    - irregular_forms: Explicitly note irregular Masculine Accusative forms, Plural forms, etc.
    - verb_tense_examples: If a verb, a dictionary with keys 'Present', 'Past', 'Perfect', 'Subjunctive I/II' and example values.
    ...
    - nominative_expression_tag: Identify if it forms a fixed nominative phrase.
    ...
    try:
        ...
        for item in items:
            try:
                validated = GermanEnrichedWord(**item)
...
async def main():
    ...
    OUTPUT_FILE = "/home/chris/wordhord/enriched_german.json"
    
    # Fetch a sample of German words from the database
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT term FROM cards WHERE language = 'german' LIMIT 100")
    words = [row[0] for row in cursor.fetchall()]
    conn.close()
    
    if not words:
        # Fallback to some high frequency words if DB is empty
        words = ["der Hund", "gehen", "das Haus", "sehen", "schön", "aufmachen", "der Mensch"]
        
    print(f"Starting enrichment for {len(words)} German words...")
